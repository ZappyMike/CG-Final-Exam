...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		BackendHandler::imGuiCallbacks.push_back([&]() {
			...
			if (ImGui::CollapsingHeader("Final Exam Controls"))
			{
				DepthOfField* temp = dofBuffer;
				IlluminationBuffer* temp2 = illuminationBuffer;
				float DL = temp->getDepthLimit();
				bool ambient = temp2->getAmbient();
				bool diffuse = temp2->getDiffuse();
				bool texture = temp2->getTexture();

				if (ImGui::SliderFloat("Depth of Field Limit", &DL, 0.0f, 1.0f))
				{
					dofBuffer->setDepthLimit(DL);
				}

				if (ImGui::Checkbox("Ambient", &ambient))
				{
					illuminationBuffer->SetAmbientBool(ambient);
				}
				if (ImGui::Checkbox("Lighting", &diffuse))
				{
					illuminationBuffer->SetDiffuseBool(diffuse);
				}
				if (ImGui::Checkbox("Texture", &texture))
				{
					illuminationBuffer->SetTextureBool(texture);
				}
		...
		Texture2D::sptr bird = Texture2D::LoadFromFile("images/BirdTexture.png");
		...
		treeMat->Shader = gBufferShader;
		treeMat->Set("s_Diffuse", tree);
		treeMat->Set("s_Specular", noSpec);
		treeMat->Set("u_Shininess", 8.0f);
		treeMat->Set("u_TextureMix", 0.0f);

		ShaderMaterial::sptr birdMat = ShaderMaterial::Create();
		birdMat->Shader = gBufferShader;
		birdMat->Set("s_Diffuse", bird);
		birdMat->Set("s_Specular", noSpec);
		birdMat->Set("u_Shininess", 8.0f);
		birdMat->Set("u_TextureMix", 0.0f);
		...
		GameObject obj2 = scene->CreateEntity("monkey_quads");
		{
			...
			obj2.get<Transform>().SetLocalPosition(0.0f, 0.0f, -20.0f);
			obj2.get<Transform>().SetLocalRotation(0.0f, 0.0f, -90.0f);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(obj2);
		}

		GameObject bird1 = scene->CreateEntity("bird");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/MidtermBird.obj");
			bird1.emplace<RendererComponent>().SetMesh(vao).SetMaterial(birdMat);
			bird1.get<Transform>().SetLocalPosition(0.0f, 0.0f, 4.0f);
			bird1.get<Transform>().SetLocalRotation(90.0f, 0.0f, 0.0f);
		}
		...
		GameObject dofBufferObject = scene->CreateEntity("dof Buffer");
		{
			dofBuffer = &dofBufferObject.emplace<DepthOfField>();
			dofBuffer->Init(width, height);

			dofBuffer->setLightPos(-illuminationBuffer->GetSunRef()._lightDirection);
			dofBuffer->setLightCol(illuminationBuffer->GetSunRef()._lightCol);
		}
		...
		while (!glfwWindowShouldClose(BackendHandler::window)) {
			...
			dofBuffer->SetLightSpaceViewProj(lightSpaceViewProj);
			dofBuffer->setCamPos(camPos);
			...
			dofBuffer->UnbindBuffer();

			shadowBuffer->BindDepthAsTexture(30);

			illuminationBuffer->ApplyEffect(gBuffer);

			temp = illuminationBuffer;

			dofBuffer->ApplyEffect(gBuffer, temp);
